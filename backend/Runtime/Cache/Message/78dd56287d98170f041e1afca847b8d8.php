...
<script src="<?php echo (ROOT_URL); ?>/Public/backend/plugins/timepicker/bootstrap-timepicker.min.js"></script>
  ...
  <div class="content-wrapper">
    ...
    <div class="col-xs-12">
        <div class="search1">
           <a href="<?php echo U('Message/Index/addMessagePage');?>"><button type="button" class="btn bg-olive btn-flat margin"><i class="fa fa-fw fa-plus"></i>添加公告消息</button></a>
           <a href="<?php echo U('Message/Index/index');?>"><button type="button" class="btn bg-purple btn-flat margin"><i class="fa fa-fw fa-refresh"></i>刷新页面</button></a>
        </div>
    </div>
    <!-- Search content end -->
    <div class="col-xs-12">
        <div class="search">
            <form id="search" action="<?php echo U('Message/Index/index');?>" method="get" >
            ...
            <div class="box-body">
              <table id="example2" class="table table-bordered table-hover">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                  <td><input type="checkbox"/><?php echo ($vo["messageid"]); ?></td>
                  <td><?php echo (mb_substr($vo["messagetitle"],0,10,'utf-8')); ?>...</td>
                  <td>
                  <?php if($vo["messagetype"] == 1 ): echo C("MESSAGETYPE.1"); endif; ?>
                  <?php if($vo["messagetype"] == 2 ): echo C("MESSAGETYPE.2"); endif; ?>
                  <?php if($vo["messagetype"] == 3 ): echo C("MESSAGETYPE.3"); endif; ?>
                  </td>
                  <td><?php echo ($vo["messagecontent"]); ?></td>
                   <td><?php echo ($vo["updatetime"]); ?></td>
                  <td><?php echo ($vo["createstaffname"]); ?></td>
                  <td><?php echo ($vo["updatetime"]); ?></td>
                  <td>
                      <a href="<?php echo U('Message/Index/editMessagepage',array('messageid'=>$vo['messageid']));?>">
                      <button class="btn bg-olive btn-flat margin">
                          <i class="fa fa-fw fa-edit"></i>编辑
                      </button>
                      </a>
                      <button value="<?php echo ($vo["messageid"]); ?>" class="btn bg-maroon btn-flat margin delete">
  ...
  $(function () {
    ...
    $(".delete").click(function(){
      ...
      if(r)
      {
         ...
         window.location.href="<?php echo U('Message/Index/delete/messageid/"+messageid+"');?>";
      }
    });
  })
</script>