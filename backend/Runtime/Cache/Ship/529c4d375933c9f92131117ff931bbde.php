<?php if (!defined('THINK_PATH')) exit();?><link rel="stylesheet" href="../../../Public/backend/css/index.css">
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<script src="<?php echo (ROOT_URL); ?>/Public/backend/plugins/timepicker/bootstrap-timepicker.min.js"></script>
  ...
  <div class="content-wrapper">
    ...
    <div class="col-xs-12">
        <div class="search1">
         <button onclick="javascript: location.href='<?php echo U('Ship/Shipping/add');?>';" type="button" class="btn bg-olive btn-flat margin"><i class="fa fa-fw fa-plus"></i>添加运费模板</button>
           <a href="<?php echo U('Ship/Shipping/index');?>"><button type="button" class="btn bg-purple btn-flat margin"><i class="fa fa-fw fa-refresh"></i>刷新页面</button></a>
        </div>
    </div>
    <!-- Search content end -->
    <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="example2" class="table table-bordered table-hover">
                ...
                <tbody>
                <?php if(is_array($template_list)): $i = 0; $__LIST__ = $template_list;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$template): $mod = ($i % 2 );++$i;?><tr>
					    <td><input type="checkbox" /><?php echo ($template["template_id"]); ?></td>
						<td><?php echo ($template["template_name"]); ?></td>
						<td><?php echo ($template["province"]); echo ($template["city"]); echo ($template["district"]); ?></td>
						<td><?php if(($template["price_way"]) == "1"): ?>按件数<?php endif; ?>
							<?php if(($template["price_way"]) == "2"): ?>按重量<?php endif; ?>
							<?php if(($template["price_way"]) == "3"): ?>按体积<?php endif; ?>
						</td>
						<td><?php if(($template["price_way"]) == "1"): echo ($template["default"]["first_num"]); endif; ?></td>
						<td><?php if(($template["price_way"]) == "1"): echo ($template["default"]["continue_num"]); endif; ?></td>
						<td><?php if(($template["price_way"]) == "2"): echo ($template["default"]["first_num"]); endif; ?></td>
						<td><?php if(($template["price_way"]) == "2"): echo ($template["default"]["continue_num"]); endif; ?></td>
						<td><?php if(($template["price_way"]) == "3"): echo ($template["default"]["first_num"]); endif; ?></td>
						<td><?php if(($template["price_way"]) == "3"): echo ($template["default"]["continue_num"]); endif; ?></td>
						<td><?php echo ($template["default"]["first_fee"]); ?></td>
						<td><?php echo ($template["default"]["continue_fee"]); ?></td>
						<td>
							<input class="small_btn_blue btn bg-olive btn-flat margin" type="button" value="查看模板" onclick="javascript: location.href='<?php echo U('info',array('template_id'=>$template[template_id]));?>';"/>
							<a class="small_btn_blue btn bg-maroon btn-flat margin" style="font: 13.3333px Arial;text-indent: 0px;text-shadow: none;letter-spacing: normal;word-spacing: normal" title="删除" href="<?php echo U('delete',array('template_id'=>$template[template_id]));?>" onclick="if(!confirm('确定删除吗？')){return false;}">删除</a>
						</td>
						<input type="hidden" name="goods_id" value="<?php echo ($vo["goods_id"]); ?>" />
					</tr><?php endforeach; endif; else: echo "暂时没有数据" ;endif; ?>
                   <!--  <tr>
                  <td><input type="checkbox"/><?php echo ($vo["articleid"]); ?></td>
                  <td><img class="articleimg" src="<?php echo ($vo["articleimg"]); ?>" /></td>
                  <td><?php echo (mb_substr($vo["articletitle"],0,10,'utf-8')); ?>...</td>
                  <td><?php echo (mb_substr($vo["articlekey"],0,10,'utf-8')); ?>...</td>
                  <td><?php echo ($vo["createtime"]); ?></td>
                  <td><?php echo ($vo["createstaffname"]); ?></td>
                  <td><?php echo ($vo["updatetime"]); ?></td>
                  <td><?php echo ($vo["updatestaffname"]); ?></td>
                  <td><?php echo ($vo["articlesource"]); ?></td>
                  <td>
                      <a href="<?php echo U('Article/Index/editarticlepage');?>?articleid=<?php echo ($vo["articleid"]); ?>">
                      <button class="btn bg-olive btn-flat margin">
                          <i class="fa fa-fw fa-edit"></i>编辑
                      </button>
                      </a>
                      <button value="<?php echo ($vo["articleid"]); ?>" class="btn bg-purple btn-flat margin recycle">
                          <i class="fa fa-fw fa-refresh"></i>回收
                      </button>
                      <button value="<?php echo ($vo["articleid"]); ?>" class="btn bg-maroon btn-flat margin delete">
  ...
  $(function () {
    ...
    $(".delete").click(function(){
      ...
      if(r)
      {
         ...
         window.location.href="<?php echo U('Article/Index/delete');?>?articleid="+articleid;
      }
    });
    $(".recycle").click(function(){
      ...
      {
         var articleid = $(this).val();
         window.location.href="<?php echo U('Article/Index/recycle');?>?articleid="+articleid;
      }
    });
