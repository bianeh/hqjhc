...
class DealController extends Controller{
    ...
    public function getallorderInfo()
    {
        ...
        $sameorderNo = I('post.sameorderno');
        $aorderinfo = $this->getallsameorderInfo($sameorderNo);
        foreach ($aorderinfo as $key => $value) 
        {
            $orderno = $value['orderNo'];
            $goodsid = $value['goodsid'];
            $GoodsInfo = $this->getGoodsInfo($goodsid);
            $brandid = $value['brandsid'];
            $BrandsInfo = $this->getBrandidInfo($brandid);   
            $aorderinfo[$key]['goodsinfo'] = $GoodsInfo;
            $aorderinfo[$key]['brandsinfo'] = $BrandsInfo;
        }
        $response['code'] = '000008';
        $response['orderinfo'] = $aorderinfo;
        $response['msg'] = '获取数据成功！';
    ...
    //获取购物车批量下单的信息
    public function getallsameorderInfo($sameorderNo)
    {
        $Orders = D('orders');
    ...
    public function index()
    {
        ...
        $response['BrandsInfo'] = $BrandsInfo;
        $response['code'] = '000008';
        $response['msg'] = '获取数据成功！';
        header('Content-type:text/html; Charset=utf8');  
        header( "Access-Control-Allow-Origin:*");  
    ...
    public function shop_deal()
    {
        $method = I('post.method');
        //支付宝支付交易
        if($method == 1)
        {
            $this->zfb_do_single_deal();
        }
        //微信支付交易
        if($method == 2)
        {
    ...
    public function wechat_do_single_deal()
    {       
            $loginName = I('get.loginName');
            $orderno = I('get.orderno');
            $dprice = floatval(I('get.price'));
            $addressid = I('get.addressid');
            $num = I('get.num');
            $price = floatval($num*$dprice);
            ...
            require_once  "/home/www/hqjhc/api/Common/Payment/wechatsdk2/WxPay.Api.php";
            require_once  "/home/www/hqjhc/api/Common/Payment/wechatsdk2/WxPay.Data.php";
            // 获取支付金额
            $UserInfo = $this->getUserInfo($loginName);
            $userid = $UserInfo['userid'];
            $arr['orderno'] = $this->create_order_no();
            $arr['otherorderno'] = $orderno;
            $arr['actualpayMoney'] = $price;
            $arr['userid'] = $userid;
            $arr['payType'] = 1;
            $arr['createTime'] = date("Y-m-d H:i:s",time());
            $arr['datasrc'] = 2;
            $arr['remarks'] = "微信线上订单交易！";
            $arr['status'] = 0;
       ...
       {    
          ...
          if($result_code == 'SUCCESS' && $return_code == 'SUCCESS')
          {
            ...
            if($count == 1)
            {
                    ...
                    $OrdersInfo = $this->getOrdersInfo($order_no);
                    ...
                    $logs_moneys['datasource'] = 6;
                    $logs_moneys['userid'] = $userid;
                    $logs_moneys['otherOrderno'] = $order_no;
    ...
    //============================支付宝交易 start ============================//
    public function zfb_do_single_deal()
    {       
            $loginName = I('get.loginName');
            $orderno = I('get.orderno');
            $dprice = floatval(I('get.price'));
            ...
            // 异步通知地址
            $notify_url = urlencode('http://118.31.45.231/api.php/Home/Deal/zfbnotify');
            // 订单标题
    ...
    //支付宝交易回调通知
       public function zfbnotify()
       {
           ...
           if($flag)
           {
                 ...
                 file_put_contents("test3333.txt",$order_no);
    ...
    //============================支付宝交易 end ==============================//
    //============================单商品交易付款 start ========================//
    public function do_single_deal()
    {   
        $loginName = I('post.loginName');
        $orderno = I('post.orderno');
        $dprice = floatval(I('post.price'));
        ...
        {   
            ...
            $arr['status'] = 1;
            $arr['datasrc'] = 3;
            $arr['remarks'] = "账户进行订单交易！";
            $this->order_settlement($arr); 
            $logs_moneys['amount'] = $price;
            $logs_moneys['moneyType'] = 1;
            $logs_moneys['accessType'] = 2;
            $logs_moneys['orderNo'] = $this->create_order_no();
            $logs_moneys['createTime'] = date("Y-m-d H:i:s",time());
            $logs_moneys['datasource'] = 5;
            $logs_moneys['userid'] = $userid;
